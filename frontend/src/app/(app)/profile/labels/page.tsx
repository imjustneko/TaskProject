...
function LabelRow({ label, onDelete, onUpdate }: {
  ...
  if (editing) return (
    <div style={{ padding: "12px 0", borderBottom: "1px solid var(--border)" }}>
      ...
      <div className="row gap-2" style={{ justifyContent: "flex-end" }}>
        <button className="btn btn-sm" onClick={() => setEditing(false)}>{t("cancel")}</button>
        <button className="btn btn-sm btn-accent" onClick={save}>{t("save")}</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="row gap-3" style={{ padding: "10px 0", borderBottom: "1px solid var(--border)" }}>
      ...
      <span className="muted" style={{ fontSize: 12 }}>{tf("label_task_count", label._count?.tasks ?? 0)}</span>
...
export default function LabelsPage() {
  ...
  const handleCreate = () => {
    ...
    createLabel.mutate({ name: name.trim(), color }, {
      onSuccess: () => { toast.show(tf("label_added", name)); setName(""); },
      onError: (e: unknown) => {
        const msg = (e as { response?: { data?: { message?: string } } })?.response?.data?.message;
        toast.show(typeof msg === "string" ? msg : t("label_add_failed"), "error");
      },
    });
  };
  ...
  return (
    <div className="view-narrow">
      <PageHeader eyebrow={t("labels_eyebrow")} title={t("labels_title")} subtitle={t("labels_subtitle")} />

      {/* Create */}
      <div className="card" style={{ marginBottom: 20 }}>
        <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 12 }}>{t("add_label_title")}</div>
      ...
      <div className="card">
        {labels.length === 0 ? (
          ...
          <LabelRow
            key={l.id}
            label={l}
            onDelete={() => deleteLabel.mutate(l.id, { onSuccess: () => toast.show(t("label_deleted")) })}
            onUpdate={(n, c) => updateLabel.mutate({ id: l.id, name: n, color: c }, { onSuccess: () => toast.show(t("label_updated")) })}
          />
        ))}
      </div>
    </div>
